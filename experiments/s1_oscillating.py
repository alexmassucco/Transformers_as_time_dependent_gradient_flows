"""
S¹ oscillating weights: deterministic rotating drift on the circle.
"""

DESCRIPTION = "S¹ oscillating D(t): clustering + orbiting on the circle"
...
_DEFAULTS = dict(T=20.0, dt=5e-3, H=[1, 5, 20], n=100)
...
def run(args):
    ...
    drift = make_drift_oscillating(d=2)
    ...
    for H in heads:
        ...
        results[H] = res
        pt_path = os.path.join(results_dir, f"s1_oscillating_H{H}.pt")
        torch.save({"s1": res, "H": H, "case": "s1_oscillating"}, pt_path)
    ...
    if make_gif:
        ...
        gif_path = os.path.join(figures_dir, f"s1_oscillating_H{H}.gif")
        save_s1_gif(traj, gif_path, color=C_CASE1, title=f"S1 oscillating (H={H})")
...
def _plot(results, heads, figures_dir, T):
    ...
    fig.tight_layout()
    path = os.path.join(figures_dir, "s1_oscillating_snapshots.pdf")
    ...
    path2 = os.path.join(figures_dir, "s1_oscillating_g2.pdf")
    ...
    path3 = os.path.join(figures_dir, "s1_oscillating_edi.pdf")
