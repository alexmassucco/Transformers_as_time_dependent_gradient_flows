"""
S² OU weights: noisy Ornstein-Uhlenbeck drift on the sphere.
"""
...
_DEFAULTS = dict(T=20.0, dt=5e-3, H=[1, 10, 100], n=200, n_mc=10, noise_var=2.0)
...
def run(args):
    ...
    for H in heads:
        ...
        results[H] = res
        pt_path = os.path.join(results_dir, f"s2_ou_H{H}.pt")
        torch.save(
            {
                "s2": res,
                "H": H,
    ...
    if make_gif:
        ...
        gif_path = os.path.join(figures_dir, f"s2_ou_H{H}.gif")
        save_s2_gif(traj, gif_path, color=C_CASE2, title=f"S2 OU (H={H})")
...
def _plot(results, heads, figures_dir, T):
    ...
    path = os.path.join(figures_dir, "s2_ou_snapshots.pdf")
    ...
    path2 = os.path.join(figures_dir, "s2_ou_g2.pdf")
    ...
    path3 = os.path.join(figures_dir, "s2_ou_edi.pdf")
